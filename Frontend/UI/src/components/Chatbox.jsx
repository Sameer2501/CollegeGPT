...
const Chatbox = () => {
  ...
  // 🧠 Auto focus input for keyboard to open
    ...
    try {
      ...
      const interval = setInterval(() => {
        if (index < cleanAnswer.length) {
          ...
          setStreamedAnswer('');
          setLoading(false);
          scrollToBottom(); // scroll after full answer
        }
      }, 20);
    } catch (error) {
      setChat((prev) => [...prev, { type: 'bot', text: 'Error getting response' }]);
  ...
  return (
    ...
    <div
      className="relative min-h-[100dvh] w-full flex flex-col bg-cover bg-center"
      style={{ backgroundImage: `url(${img})` }}
    >
      <div className='mb-[80px]'>
        <img src={img1} alt="Chatbot" className="absolute top-4 left-4 w-16 h-16" />
